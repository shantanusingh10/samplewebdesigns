...
function play(){
	...
	for(var i=0;i<4;i++){
		while(1){
			...
			for(var j=0;j<renderedOptions.length;j++){
				if(randomIndex == renderedOptions[j]){
					flag = 1;
					break;
				}
			}
			if(!flag){
				containerDiv.innerHTML+="<div class='option'> " + options[randomIndex] + "</div>";
				renderedOptions.push(randomIndex);
				break;
			}
			else{
				continue;
...
function func1(){
	...
	containerDiv.innerHTML+="<span id='result'>Incorrect answer. Right answer is " + randomElementNum+"</span>";
	containerDiv.innerHTML+="<div id='playagain'>Play Again</div>";
	containerDiv.innerHTML+="<div id='buttons'><button id='yes-btn' onclick='play()' class='btn btn-primary'>Yes</button><button onclick='gameOver()' id='no-btn' class='btn btn-danger'>No</button></div>";
}
...
function func2(){
	...
	var containerDiv = document.getElementById("container");
	var options = document.querySelectorAll(".option");
	containerDiv.innerHTML+="<span id='result'>Correct answer</span>";
	containerDiv.innerHTML+="<div id='playagain'>Play Again</div>";
	containerDiv.innerHTML+="<div id='buttons'><button id='yes-btn' onclick='play()' class='btn btn-primary'>Yes</button><button onclick='gameOver()' id='no-btn' class='btn btn-danger'>No</button></div>";
}
...
function setListeners(){
	...
	var options = document.querySelectorAll(".option");
	for(var i=0;i<4;i++){
		options[i].removeEventListener("click", func1);
		options[i].removeEventListener("click", func2);
			
	}
	for(var i=0;i<4;i++){
		if(options[i].innerText == randomElementNum){
			options[i].addEventListener("click",func2);
		}
		else options[i].addEventListener("click",func1);
	}		
...
function gameOver(){
	...
	containerDiv.innerHTML+="<h1>Game Over</h1>";	
}
